...
class CarlaEnvironmentWrapper(EnvironmentWrapper):
    def __init__(self, num_speedup_steps=30, require_explicit_reset=True, is_render_enabled=False,
        ...
        if self.segment_camera: self.settings.add_sensor(
            self.create_camera(self.segment_camera_name, 'SemanticSegmentation'))
    ...
    def _open_server(self):
        ...
        with open(self.log_path, "wb") as out:
            # cmd = ["docker run -p 2000-2002:2000-2002 --gpus 0 --rm carlasim/carla:0.8.4 " \
            #       "/bin/bash CarlaUE4.sh", self.map,
    ...
    def _update_state(self):
        ...
        if self.rgb_camera:
            self.observation['rgb_image'] = self.encode_obs(self.process_image(sensor_data[self.rgb_camera_name].data))
        ...
        if self.segment_camera and self.depth_camera and self.enable_coalesced_output:
            self.observation['coalesced_data'] = coalesce_depth_and_segmentation(
                self.observation['segmented_image'], self.class_grouping, self.observation['depth_map'],
                self.max_depth_value)

        if self.segment_camera and (self.autocolor_the_segments or self.is_render_enabled):
            self.observation['colored_segmented_image'] = convert_segmented_to_rgb(carla_config.colors_segment,
        ...
        # print('REWARD: %.2f, ACTIONS: %s' % (self.reward, action_p))
        ...
        self.observation = self.observation['rgb_image']
    ...
    def save_screenshots(self):
        ...
        if self.depth_camera:
            save_depthmap_as_16bit_png(self.depth_img_path + filename + ".png", self.observation['depth_map'],
                                       self.max_depth_value, 0.95)  # Truncating sky as 0
