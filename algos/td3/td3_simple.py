...
class ReplayBuffer:
    ...
    def sample(self, batch_size=32):
        ...
        batch = {"obs1": np.array(state).astype(np.float32),
                "obs2": np.array(state_).astype(np.float32),
...
class Td3:
    def __init__(self, env_name, port=2000, gpu=0, train_step=2000, evaluation_step=1000, max_ep_len=1000,
                 polyak=0.995, start_steps=1000, batch_size=100, replay_size=50000,
                ...
                for j in range(step_episode):
                    ...
                    if debug_mode:
                        with self.summary.as_default():
                            tf.summary.scalar("loss_q1", q1_loss, self.cur_tensorboard_step)
                            tf.summary.scalar("loss_q2", q2_loss, self.cur_tensorboard_step)
                            tf.summary.scalar("loss_q", q_loss, self.cur_tensorboard_step)
                            tf.summary.histogram("q1", q1, self.cur_tensorboard_step)
                            tf.summary.histogram("q2", q2, self.cur_tensorboard_step)
                            tf.summary.histogram("pi_targ", pi_targ, self.cur_tensorboard_step)
                            tf.summary.histogram("pi_a2", a2, self.cur_tensorboard_step)
                    ...
                    if j % self.policy_delay == 0:
                        ...
                        if debug_mode:
                            with self.summary.as_default():
                                tf.summary.histogram("pi", pi, self.cur_tensorboard_step)
                                tf.summary.histogram("q1_pi", q1_pi, self.cur_tensorboard_step)
                                tf.summary.scalar("loss_pi", pi_loss, self.cur_tensorboard_step)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='td3_pendulum')
